...
public:
     panel_tile_info ( sdl2_backend * sdl, const std::pair<int,int> region_loc, const std::pair<int,int> mouse_loc ) {
          ...
          info_windows.push_back ( std::make_unique<terrain_info_window> ( sdl, "Terrain Information", true, region_x, region_y ) );
          add_structure_panels(); // Settler panels are called from here
          info_windows.push_back ( std::make_unique<build_options_window> ( sdl, "Build Options", true, region_x, region_y ) );
     }
     ...
     bool render ( const std::pair<int,int> mouse_loc, bool left_click, bool right_click ) {
	  ...
	  bool in_any_window = false;
          ...
          if (!in_any_window and count>10 and (left_click or right_click)) escaping = true;
     ...
     void add_structure_panels() {
          ...
          for ( const position_component * pos : positions ) {
               ...
               if ( settler == nullptr ) {
                    ...
                    info_windows.push_back ( std::make_unique<settler_info_window> ( SDL, "", true, settler ) );
               }
          }
