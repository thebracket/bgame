...
enum skill_test_result { CRITICAL_FAIL, FAIL, SUCCESS, CRITICAL_SUCCCESS };

inline skill_test_result skill_roll(const int skill_level, const int modifiers, const int difficulty) {
    ...
    if (roll < difficulty) return FAIL;
    return SUCCESS;
}

inline int get_skill_level(const game_stats_component * entity_stats, const string skill_name) {
    // TODO: A more optimal search!
    int skill_level = 0;
    for (const skill_t &skill : entity_stats->skills) {
	if (skill_name == skill.skill_name) skill_level = skill.skill_level;
    }
    return skill_level;
}

inline int get_skill_modifier(const game_stats_component * entity_stats, const string skill_name) {
    int modifier = 0;
