...
void spread_flow( vector<tuple<short,int,int>> &new_cells, unordered_set<int> &visited, const int &x, const int &y, const short &distance, unordered_map<int, bool> &walkable ) {
    ...
    if (world::current_region->revealed[index]==false) return;
    if (visited.find( index ) != visited.end()) return;
    if (walkable.find( index ) != walkable.end()) return;
    if (world::current_region->tiles[ index].base_tile_type == tile_type::WATER ) return;
    if (x < 0) return;
...
shared_ptr< navigation_path > find_path ( pair< int, int > start, pair< int, int > end, unordered_map<int, bool> &walkable )
{
	...
	if (last_direction!= 2) options[ flow_map[ world::current_region->idx ( current.first-1, current.second ) ] ] = 1;
	if (last_direction!= 1) options[ flow_map[ world::current_region->idx ( current.first+1, current.second ) ] ] = 2;
	if (last_direction!= 4) options[ flow_map[ world::current_region->idx ( current.first, current.second-1 ) ] ] = 3;
	if (last_direction!= 3) options[ flow_map[ world::current_region->idx ( current.first, current.second+1 ) ] ] = 4;
