...
void perlin_noise::set ( double _persistence, double _frequency, double _amplitude, int _octaves, int _randomseed )
{
     persistence_d = _persistence;
     frequency_d = _frequency;
     amplitude_d  = _amplitude;
...
double perlin_noise::total ( double i, double j ) const
{
     ...
     for ( int k = 0; k < octaves_i; k++ ) {
          t += get_value ( j * freq + random_seed_i, i * freq + random_seed_i ) * _amplitude;
