    ...
    void load_vox(const std::string &filename, const std::string &tag)
    {
        ...
        for (auto i=0; i<parsed.numModels; ++i) {
            ...
            for (auto j=0; j<voxel_count; ++j) {
                ...
                auto color = get_color(parsed.palette, current_voxel.colorIndex);

                const float r = ((float)((color & 0xff000000) >> 24) / 255.0f);
                const float g = ((float)((color & 0x00ff0000) >> 16) / 255.0f);
                const float b = ((float)((color & 0x0000ff00) >> 8) / 255.0f);

                constexpr float scale = 0.05f;

                subvoxel v{
                        (float)current_voxel.x * scale,
                        (float)current_voxel.y * scale,
                        (float)current_voxel.z * scale,
                        r,
                        g,
                        b
                };
