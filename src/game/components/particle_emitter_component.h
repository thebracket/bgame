...
public:
	...
	particle_emitter_component(const string &msg, const int &time,
			const int &freq, const int col = 7) :
			message(msg), ttl(time), emits_one_in_x(freq), color(col)
	{
	}
	...
	{
		save_common_component_properties<particle_emitter_component>(lbfile,
				*this);
	...
	{
		load_common_component_properties<particle_emitter_component>(lbfile,
				*this);
