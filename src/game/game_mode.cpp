...
private:
	...
	void render_cursor(sdl2_backend * SDL, pair<int, int> &screen_size,
			SDL_Rect &viewport, position_component3d * camera_pos)
	{
	...
	void render_tooltip(sdl2_backend * SDL, const location_t &loc,
			const std::pair<int, int> mouse)
	{
		if (tooltip_window)
		{
