		...
		const double& angle)
{
	...
	if (pos->facing == NORTH
			and (angle > TWO_TWENTY_FIVE and angle < THREE_FIFTEEN))
		return true;
	if (pos->facing == SOUTH
			and (angle > FORTY_FIVE and angle < ONE_THIRTY_FIVE))
		return true;

	if (pos->facing == EAST and (angle > THREE_FIFTEEN or angle < FORTY_FIVE))
		return true;
	if (pos->facing == WEST
			and (angle > ONE_THIRTY_FIVE and angle < TWO_TWENTY_FIVE))
		return true;

	return false;
}

void viewshed_system::scan_radius_for_visibility(viewshed_component * view,
		const position_component3d * pos)
{
			...
			sweep_degrees_radians)
	{
		...
		if (!is_facing_this_way(pos, angle))
			this_radius = radius / 4;
...
void viewshed_system::scan_radius_penetrating(viewshed_component * view,
		const position_component3d * pos)
{
	constexpr double sweep_degrees_radians = 2 * DEGRAD;
	constexpr double three_sixy_degrees_radians = 360 * DEGRAD;
	...
	{
		double this_radius = radius;
		if (!is_facing_this_way(pos, angle) == true)
			this_radius = 2.0;

		pair<int, int> destination = project_angle(x, y, this_radius, angle);
		for (int lz = z - this_radius; lz < z + this_radius; ++lz)
		{
			line_func_3d(destination.first, destination.second, lz, x, y, z,
					[view,current_region] (int tx, int ty, int tz)
					{
						const int index = get_tile_index(tx,ty,tz);
						if (tx >=0 and tx < REGION_WIDTH and ty>=0 and ty<REGION_HEIGHT and tz>0 and tz<REGION_DEPTH)
						{
