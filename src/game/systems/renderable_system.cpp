...
void renderable_system::tick ( const double &duration_ms )
{
     ...
     if ( changes ) {
	  ...
	  const position_component3d * camera_pos = ECS->find_entity_component<position_component3d> ( world::camera_handle );
	  ...
	  for ( const position_component3d &pos : *positions ) {
		...
		renderable_component * render_info = ECS->find_entity_component<renderable_component> ( pos.entity_id );
