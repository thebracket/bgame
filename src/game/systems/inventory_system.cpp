		...
		position_component3d * pos)
{
	...
	{
		available_item item
		{ name->debug_name, pos->pos, name->entity_id };
		inventory[name->debug_name] = vector<available_item>
		{ item };
	}
	else
	{
...
void inventory_system::register_inventory_stored(item_storage_component * store)
{
	debug_name_component * name = ECS->find_entity_component<
			debug_name_component>(store->entity_id);
	position_component3d * pos =
			ECS->find_entity_component<position_component3d>(
					store->container_id);
...
{
	debug_name_component * name = ECS->find_entity_component<
			debug_name_component>(carried->entity_id);
	position_component3d * pos =
			ECS->find_entity_component<position_component3d>(
...
{
	debug_name_component * name = ECS->find_entity_component<
			debug_name_component>(pos->entity_id);
	store_inventory(name, pos);
}
...
void inventory_system::tick(const double& duration_ms)
{
	...
	vector<item_changed_message> * item_changes =
			game_engine->messaging->get_messages_by_type<item_changed_message>();
	...
	vector<item_component> * items =
			ECS->find_components_by_type<item_component>();
	for (const item_component &item : *items)
	{
		if (!item.claimed)
		{
			item_storage_component * store = ECS->find_entity_component<
					item_storage_component>(item.entity_id);
			item_carried_component * carried = ECS->find_entity_component<
					item_carried_component>(item.entity_id);
			position_component3d * on_ground = ECS->find_entity_component<
					position_component3d>(item.entity_id);
