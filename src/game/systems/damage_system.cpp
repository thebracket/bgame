...
void damage_system::tick ( const double& duration_ms )
{
	...
	if (!damage.deleted) {
	    game_health_component * health = ECS->find_entity_component<game_health_component>( damage.target_entity );
	    if ( health != nullptr ) {
	...
	if ( calendar->minute % 30 == 0 ) {
	    ...
	    vector<game_health_component> * health = ECS->find_components_by_type<game_health_component>();
		...
		if (h.current_hit_points < h.max_hit_points) {
			...
			position_component3d * pos = ECS->find_entity_component<position_component3d>( h.entity_id );
