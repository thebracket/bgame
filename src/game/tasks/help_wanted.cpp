...
void load_help_wanted(fstream& lbfile)
{
	...
	for (int i = 0; i < number_of_jobs; ++i)
	{
		...
		job_t job
		{ static_cast<job_type_e>(job_type), job_id,
		...
		for (int j = 0; j < number_of_steps; ++j)
		{
			...
			job.steps.push_back(job_step_t
			{ static_cast<job_step_e>(step_type), target_x, target_y, target_z,
					component_id, requires_skill, skill_name,
					placeholder_structure_id, required_skill_difficulty });
		}
