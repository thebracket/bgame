...
{
     map<int,int> options;
     vector<provisions_component> * provisions = ECS->find_components_by_type< provisions_component >();
     for ( const provisions_component &provision : *provisions ) {
          if ( provision.provided_resource == provision_type ) {
               ...
               if ( provision.provided_resource != 3 or provision.provides_quantity==0 ) {
                    position_component3d * pos = ECS->find_entity_component<position_component3d> ( provision.entity_id );
