...
int downhill_target(const int &x, const int &y, const int &hidx,
		heightmap_t * heightmap, water_level_map_t * water)
{
...
std::unique_ptr<water_level_map_t> perform_hydrology(heightmap_t * heightmap,
		engine::random_number_generator * rng)
{
	...
	std::unique_ptr<water_level_map_t> water_ptr = std::make_unique<
			water_level_map_t>();
	...
	for (int pass = 0; pass < 1; ++pass)
	{
		...
		while (!stable)
		{
			...
			for (const uint8_t &w : *water_ptr)
				water_copy->push_back(w);
			for (int y = 1; y < height - 1; ++y)
			{
				for (int x = 1; x < width - 1; ++x)
				{
					...
					if (water_ptr->operator[](hidx) > 0)
					{
						...
						const int destination_idx = downhill_target(x, y, hidx,
								heightmap, water_ptr.get());
						if (destination_idx == hidx or destination_idx == -1)
						{
							// There's nowhere to go from here.
							if (heightmap_copy->operator[](hidx)
									> min_heights->operator[](hidx))
							{
								heightmap_copy->operator[](hidx) =
										heightmap_copy->operator[](hidx) - 1;
								stable = false;
								++changes;
							}
							++stuck;
						...
						else
						{
							...
							if (water_copy->operator[](destination_idx) < 253)
								water_copy->operator[](destination_idx) =
										water_copy->operator[](destination_idx)
												+ 1;

							if (water_copy->operator[](hidx) > 1)
								water_copy->operator[](hidx) =
										water_copy->operator[](hidx) - 1;
			...
			heightmap->clear();
			std::copy(heightmap_copy->begin(), heightmap_copy->end(),
					std::back_inserter(*heightmap));
			water_ptr->clear();
			...
			++pass_count;
			std::cout << "Pass " << pass << "~" << pass_count << " (changes: "
					<< changes << ", dry: " << dry << ", unmoving: " << stuck
					<< ") \n";
			changes = 0;
			dry = 0;
		}
