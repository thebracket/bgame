...
biome_map_t make_biome_map(heightmap_t* heightmap,
		engine::random_number_generator* rng, water_level_map_t* water)
{
	...
	png_writer png("world/voronoi.png", WORLD_WIDTH * REGION_WIDTH,
	...
	for (int i = 0; i < number_of_biomes; ++i)
	{
		...
		const double temperature_c_base = -40.0
				+ ((static_cast<double>(points[i].second) / height) * 80.0)
				- (BIOME_AVERAGE_ALTITUDE / 100.0);
		...
		if (result.biomes[i].climate == tile_climate::COLD
				or result.biomes[i].climate == tile_climate::COOL
				or result.biomes[i].climate == tile_climate::TEMPERATE
				or result.biomes[i].climate == tile_climate::WARM)
		{
			...
			else
			{
				if (BIOME_AVERAGE_ALTITUDE < average_altitude)
				{
					...
					{
					case 1:
						result.biomes[i].biome_type = biome_types::FLATLAND;
						break;
					case 2:
						result.biomes[i].biome_type = biome_types::WOODS;
						break;
					case 3:
						result.biomes[i].biome_type = biome_types::FOREST;
						break;
					}
				}
				else
				{
					...
					{
					case 1:
						result.biomes[i].biome_type = biome_types::HILLS;
						break;
					case 2:
						result.biomes[i].biome_type = biome_types::WOODS;
						break;
		...
		std::cout << i << " ... Altitude Average: " << BIOME_AVERAGE_ALTITUDE
				<< "/" << average_altitude << ". Wetness: " << AVERAGE_WETNESS
				<< ". Distance: " << distance_from_start << ". Temp: "
				<< temperature_c_base << "C\n";
