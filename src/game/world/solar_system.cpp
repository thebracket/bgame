...
void solar_system_t::load(std::fstream &lbfile)
{
	...
	lbfile.read(reinterpret_cast<char *>(&binary_system),
			sizeof(binary_system));
	lbfile.read(reinterpret_cast<char *>(&inhabited_by_man),
			sizeof(inhabited_by_man));
...
{
	lbfile.write(reinterpret_cast<const char *>(&universe_idx),
			sizeof(universe_idx));
	lbfile.write(reinterpret_cast<const char *>(&universe_x),
			sizeof(universe_x));
	lbfile.write(reinterpret_cast<const char *>(&universe_y),
			sizeof(universe_y));
	lbfile.write(reinterpret_cast<const char *>(&system_class),
			sizeof(system_class));
	lbfile.write(reinterpret_cast<const char *>(&binary_system),
			sizeof(binary_system));
	lbfile.write(reinterpret_cast<const char *>(&inhabited_by_man),
			sizeof(inhabited_by_man));
...
std::pair<SDL_Color, unsigned char> solar_system_renderable(
		const solar_system_t &system)
{
	if (!system.inhabited_by_man)
	{
		...
		case M:
			...
			{ 128, 128, 0, 0 }, 249);
		default:
			return std::make_pair(SDL_Color
			{ 255, 255, 255, 0 }, 15);
		}
	}
