...
enum nested_enum
{
	NONE, STRUCTURE, STARTING_PROFESSION, ITEM, REACTION
};
...
void parse_raw_description(const vector<string> &chunks)
{
	...
	unique_ptr<raw_description> desc = make_unique<raw_description>(
			description);
...
void parse_raw_power_generator(const vector<string> &chunks)
{
	...
	unique_ptr<raw_power_generator> gen = make_unique<raw_power_generator>(
			power_mode, amount);
...
void parse_raw_power_battery(const vector<string> &chunks)
{
	...
	unique_ptr<raw_power_battery> batt = make_unique<raw_power_battery>(
			capacity);
...
void parse_raw_modifier(const vector<string> &chunks)
{
	...
	unique_ptr<raw_modifier> name = make_unique<raw_modifier>(chunks[1],
			stat_mod);
	current->children.push_back(std::move(name));
}
...
void parse_raw_clothing_slot(const vector<string> &chunks)
{
	...
	unique_ptr<raw_clothing_slot> slot = make_unique<raw_clothing_slot>(
			slot_name);
...
void parse_raw_clothing(const vector<string> &chunks)
{
	...
	if (chunks[0] == "CLOTHING_FEMALE")
		gender = 2;
	unique_ptr<raw_clothing> slot = make_unique<raw_clothing>(clothing_slot,
			clothing_item, gender);
	current->children.push_back(std::move(slot));
}
...
void parse_structure(const vector<string> &chunks)
{
	...
	if (chunks[0] == "SA-NAME")
	{
		raw_settler_action * tmp =
				static_cast<raw_settler_action *>(current_sub_chunk.get());
	...
	{
		current_sub_chunk->children.push_back(
				make_unique<raw_component>(chunks[1], std::stoi(chunks[2])));
		return;
	}
	if (chunks[0] == "BUILDABLE_SKILL")
	{
		current_sub_chunk->children.push_back(
				make_unique<raw_skill_required>(chunks[1], std::stoi(chunks[2]),
						std::stoi(chunks[3])));
		return;
	}
	...
	std::cout
			<< "WARNING: GOT TO END OF STRUCTURE CHUNK READER WITH NO MATCH FOR ["
			<< chunks[0] << "]\n";
}
...
void parse_whole_chunk(const vector<string> &chunks)
{
	...
	if (chunks[0] == "COLOR")
	{
		...
		detail::colors[color_name] = engine::vterm::color_t
	...
	std::cout << "WARNING: GOT TO END OF CHUNK READER WITH NO MATCH FOR ["
			<< chunks[0] << "]\n";
}
...
int create_item_from_raws(const string &name)
{
	...
	for (const unique_ptr<base_raw> &i : finder->second->children)
	{
		...
		{
			raw_clothing_color * rwc =
					static_cast<raw_clothing_color *>(i.get());
...
pair<string, base_raw *> get_random_starting_profession()
{
	...
	int i = 0;
	for (auto it = detail::starting_professions.begin();
			it != detail::starting_professions.end(); ++it)
	{
		if (i == die_roll)
		{
			return std::make_pair(to_proper_noun_case(it->first),
					it->second.get());
		}
		++i;
	}
...
string get_random_first_name(const short int& gender)
{
	...
	{
		return line_from_text_file("../raw/first_names_female.txt",
				game_engine->rng.roll_dice(1, NUMBER_OF_FEMALE_FIRST_NAMES) - 1);
	}
	else
	{
		return line_from_text_file("../raw/first_names_male.txt",
				game_engine->rng.roll_dice(1, NUMBER_OF_MALE_FIRST_NAMES) - 1);
	}
}

string get_random_last_name()
{
	const int die_roll = game_engine->rng.roll_dice(1, NUMBER_OF_LAST_NAMES)
			- 1;
		...
		const string &structure_name)
{
	...
	for (auto it = detail::reactions.begin(); it != detail::reactions.end();
			++it)
	{
		const bool has_possible_reactions =
				it->second->has_reactions_for_workshop(structure_name);
