...
{
	raw_particle_emitter()
	{
		type = PARTICLE_EMITTER;
	}
	;
	raw_particle_emitter(const string &msg, const int &time, const int &freq,
			const chat_emote_color_t col) :
			message(msg), ttl(time), frequency(freq), color(col)
	{
		type = PARTICLE_EMITTER;
	}
	string message;
			...
			const bool placeholder = false) const
	{
		if (!placeholder)
			ECS->add_component(parent,
					particle_emitter_component(message, ttl, frequency, color));
	}
