...
struct renderable_component
{
	...
	renderable_component(const unsigned char &g, const color_t &f,
			const color_t &b, const int &tile, const int &render_layer = 2,
			const bool &placeholder = false, const bool &compose = false) :
			glyph(g), foreground(f), background(b), tile_idx(tile), layer(
					render_layer), translucent(placeholder), composite(compose)
	{
	}
