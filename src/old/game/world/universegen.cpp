...
void universe_generator::make_universe()
{
	...
	for (int i = 0; i < numer_of_systems; ++i)
	{
		...
		while (!good)
		{
			...
			system.universe_x = game_engine->rng.roll_dice(1,
					UNIVERSE_WIDTH - 1);
			system.universe_y = game_engine->rng.roll_dice(1,
					UNIVERSE_HEIGHT - 1);
			system.universe_idx = universe_idx(system.universe_x,
					system.universe_y);
			auto finder = universe->solar_systems.find(system.universe_idx);
			if (finder == universe->solar_systems.end()
					and ((system.universe_x != UNIVERSE_WIDTH / 2)
							or (system.universe_y != UNIVERSE_HEIGHT / 2)))
			{
				good = true;
...
void universe_generator::add_planetary_bodies(solar_system_t& system)
{
	...
	for (int i = 0; i < n_bodies; ++i)
	{
		if (system.bodies[i].body_type == HABITABLE_PLANET
				or system.bodies[i].body_type == SPACE_STATION)
			habitable = true;
	}
	if (!habitable)
	{
...
void universe_generator::add_planetary_body(int i, solar_system_t& system)
{
	...
	{
	case 1:
		body.body_type = ICE_WORLD;
		break;
	case 2:
		body.body_type = VOLCANIC_WORLD;
		break;
	case 3:
		body.body_type = GAS_CLOUD;
		break;
	case 4:
		body.body_type = ASTEROIDS;
		break;
	case 5:
		body.body_type = WARP_HUSK;
		break;
	case 6:
		body.body_type = HABITABLE_PLANET;
		break;
	case 7:
		body.body_type = SPACE_STATION;
		break;
	}
	system.bodies.push_back(body);
}
	...
	for (int y = 0; y < UNIVERSE_HEIGHT; ++y)
	{
		for (int x = 0; x < UNIVERSE_WIDTH; ++x)
		{
			...
			if (distance < 15 and finder != universe->solar_systems.end())
				finder->second.inhabited_by_man = true;
		}
	}
}
	...
	for (int y = 0; y < UNIVERSE_HEIGHT; ++y)
	{
		for (int x = 0; x < UNIVERSE_WIDTH; ++x)
		{
			...
			if (distance < 15 and finder != universe->solar_systems.end())
			{
				...
				{
				case 1:
					finder->second.owner = ARSAK;
					break;
				case 2:
					finder->second.owner = CARVAZ;
					break;
				case 3:
					finder->second.owner = HERITEZ;
					break;
				case 4:
					finder->second.owner = LAMINTREZ;
					break;
				case 5:
					finder->second.owner = ERENTAR;
					break;
				case 6:
					finder->second.owner = SYL;
					break;
				}
			}
		}
