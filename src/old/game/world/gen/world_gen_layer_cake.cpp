...
std::unique_ptr<planet_t> make_world_layers(heightmap_t* base_map,
		...
		biome_map_t * biomes)
{
	...
	std::cout << "Lowest: " << lowest_ground << ", highest: " << highest_ground
	...
	{
		std::unique_ptr<marching_squares_map_t> ramps_tmp = marching_squares(
				&bands, [i] (int n)
				{
					bool result = false;
					if ( n > i ) result = true;
		...
		for (const uint8_t &tmp : *ramps_tmp)
		{
			...
			if (tmp == 9)
				ramps[idx + 1] = true;
			if (tmp == 3)
				ramps[idx + (WORLD_WIDTH * REGION_WIDTH)] = true;
			++idx;
		}
	}
	...
	// Save a PNG
	png_writer png("world/heights.png", (WORLD_WIDTH * REGION_WIDTH) + 1,
			(WORLD_HEIGHT * REGION_HEIGHT) + 1);
		...
		//{
			...
			for (uint8_t y = 0; y < REGION_HEIGHT; ++y)
			{
				for (uint8_t x = 0; x < REGION_WIDTH; ++x)
				{
					...
					{
						tile_t * tile = planet->get_tile(location_t
						{ region_index, x, y, Z });
					...
					bottom = planet->get_tile(location_t
					{ region_index, x, y, z });
					bottom->flags.set(TILE_OPTIONS::SOLID);
					bottom->base_tile_type = tile_type::FLAT;
					bottom->ground = tile_ground::IGNEOUS;
