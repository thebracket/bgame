...
{
	std::unique_ptr<vector<bool>> threshold = std::make_unique<vector<bool>>(
	...
	std::unique_ptr<marching_squares_map_t> result_ptr = std::make_unique<
			marching_squares_map_t>(NUMBER_OF_TILES_IN_THE_WORLD);
	...
	for (int y = 0; y < (WORLD_HEIGHT * REGION_HEIGHT) - 1; ++y)
	{
		for (int x = 0; x < (WORLD_WIDTH * REGION_WIDTH) - 1; ++x)
		{
			...
			uint8_t value = 0;
			if (threshold->operator[](TL))
				value += 1;
			if (threshold->operator[](TR))
				value += 2;
			if (threshold->operator[](BR))
				value += 4;
			if (threshold->operator[](BL))
				value += 8;
