...
void make_noisy_heightmap(heightmap_t* base_map,
		engine::random_number_generator * rng)
{
	...
	{
		const double Y = y;
		const double north_bias = ((Y / height) * 0.7) + 0.3;
		for (int x = 0; x < width; ++x)
		{
			const double X = x;
			const double mtn_noise_pixel = (mountains.get_height(X, Y) + 1.0)
					* 300.0;
			double mixer_noise_pixel = (mixer.get_height(X * 0.25, Y * 0.25)
					+ 0.5);
			if (mixer_noise_pixel < 0.0)
				mixer_noise_pixel = 0.0;
			if (mixer_noise_pixel > 1.0)
				mixer_noise_pixel = 1.0;
