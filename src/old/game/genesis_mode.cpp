...
pair<engine::return_mode, unique_ptr<engine::base_mode> > genesis_mode::tick(
...
void genesis_mode::intro_quote()
{
	...
	vterm::print((size.first / 2) - msg1.size() / 2, (size.second / 2) - 3,
			msg1, color_t
			{ 255, 255, 255 }, color_t
			{ 0, 0, 0 });
	vterm::print((size.first / 2) - msg2.size() / 2, (size.second / 2) - 2,
			msg2, color_t
			{ 255, 255, 255 }, color_t
			{ 0, 0, 0 });
	vterm::print((size.first / 2) - msg3.size() / 2, (size.second / 2) - 1,
			msg3, color_t
			{ 255, 255, 255 }, color_t
			{ 0, 0, 0 });
	vterm::print((size.first / 2) - msg4.size() / 2, (size.second / 2), msg4,
			color_t
			{ 255, 255, 255 }, color_t
			{ 0, 0, 0 });
	vterm::print((size.first / 2) - msg5.size() / 2, (size.second / 2) + 1,
			msg5, color_t
			{ 255, 255, 255 }, color_t
			{ 0, 0, 0 });
	vterm::print((size.first / 2) - msg6.size() / 2, (size.second / 2) + 3,
			msg6, color_t
			{ 255, 255, 0 }, color_t
			{ 0, 0, 0 });
}
...
void genesis_mode::udisplay(const double time_elapsed)
{
	...
	vterm::print((UNIVERSE_WIDTH / 2) + 1, UNIVERSE_HEIGHT / 2, message, color_t
	{ 0, 255, 255 }, color_t
	{ 0, 0, 0 });

	const string msg1
	{ "This is just a small part of the universe: Galactic Core." };
	const string msg2
	{ "In the late 21st century, Man spread across the core," };
	const string msg3
	{ "spreading ruin in its wake." };
	const string msg4
	{ "Worlds burned, and the only cure for war was bigger guns." };
	const string msg5
	{ "Six Houses rose, and decimated the galaxy for their ideology." };
	const string msg6
	{ "Democracy, Trade, Perfection, Magick, Prayer, Nobility." };
	const string msg7
	{ "All good ideas, billions died for their realization." };

	pair<int, int> size = vterm::get_screen_size();
	if (total_time > 500.0)
		vterm::print((size.first / 2) - msg1.size() / 2, 0, msg1, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
	if (total_time > 1000.0)
	{
		vterm::print((size.first / 2) - msg2.size() / 2, 1, msg2, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
		if (!spread_humanity)
		{
	...
	if (total_time > 1500.0)
		vterm::print((size.first / 2) - msg3.size() / 2, 2, msg3, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
	if (total_time > 2000.0)
	{
		vterm::print((size.first / 2) - msg4.size() / 2, 3, msg4, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
		unigen.spread_ownership();
	}
	if (total_time > 2500.0)
		vterm::print((size.first / 2) - msg5.size() / 2, 4, msg5, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
	if (total_time > 3000.0)
		vterm::print((size.first / 2) - msg6.size() / 2, 5, msg6, color_t
		{ 0, 255, 255 }, color_t
		{ 0, 0, 0 });
	if (total_time > 3500.0)
		vterm::print((size.first / 2) - msg7.size() / 2, 6, msg7, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
...
void genesis_mode::udisplay2(const double time_elapsed)
{
	...
	const string msg1
	{ "In 2520, a leader named Blue came to power in Eden." };
	const string msg2
	{ "She brokered a truce, and war-scarred Eden enjoyed some peace." };
	const string msg3
	{ "But Eden was shattered, and the people were slowly dying." };
	const string msg4
	{ "Great arks were built, evacuees chosen by lottery: " };
	const string msg5
	{ "The 'A' ark would carry the best and brightest. " };
	const string msg6
	{ "The 'C' ark would carry harmless criminals. " };
	const string msg7
	{ "The 'B' ark would carry everyone else - " };
	const string msg8
	{ "the wasted; insurance adjusters, hairdressers," };
	const string msg9
	{ "and that weird guy who is ALWAYS in the pub." };
	...
	if (total_time > 2000.0)
		...
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
	if (total_time > 2200.0)
		vterm::print((size.first / 2) - msg6.size() / 2, 5, msg6, color_t
		{ 255, 255, 255 }, color_t
		{ 0, 0, 0 });
	if (total_time > 2400.0)
		vterm::print((size.first / 2) - msg7.size() / 2, 6, msg7, color_t
		{ 255, 255, 0 }, color_t
		{ 0, 0, 0 });
	if (total_time > 2500.0)
		vterm::print((size.first / 2) - msg8.size() / 2, 7, msg8, color_t
		{ 255, 255, 0 }, color_t
		{ 0, 0, 0 });
	if (total_time > 2600.0)
		vterm::print((size.first / 2) - msg9.size() / 2, 8, msg9, color_t
		{ 255, 255, 0 }, color_t
		{ 0, 0, 0 });
	...
	if (total_time > 1900.0)
	{
		...
		vterm::print(starting_x + 1, starting_y, message2, color_t
		{ 0, 255, 255 }, color_t
		{ 0, 0, 0 });
	}
	if (total_time > 3000.0)
	{
		...
		std::cout << "Destination: " << starting_x << ", " << starting_y
				<< "\n";
		geometry::line_func(current_x, current_y, starting_x, starting_y,
...
void genesis_mode::travel(const double time_elapsed)
{
	...
	const string msg1
	{ "Due to 'technical difficulties', the B ark launches first." };
	const string msg2
	{ "The crew go to sleep, and Cordex (the ship's AI) flies the ship." };
	const string msg3
	{ "At first, it all goes rather well." };
	const string msg4
	{ "We've been hit! Sensors wonky, entering emergency mode!" };
	const string msg5
	{ "We've made it! Scanning the system..." };
	pair<int, int> size = vterm::get_screen_size();
	vterm::print((size.first / 2) - msg1.size() / 2, 0, msg1, color_t
	{ 0, 255, 255 }, color_t
	{ 0, 0, 0 });
	...
	const string message = "<- You are here";
	vterm::print(current_x + 1, current_y, message, color_t
	{ 0, 255, 255 }, color_t
	{ 0, 0, 0 });
}
...
void genesis_mode::system_scan(const double time_elapsed)
{
	total_time += time_elapsed;
	const string msg1
	{ "We've dropped into an unknown system. Scanning..." };
	pair<int, int> size = vterm::get_screen_size();
	vterm::print((size.first / 2) - msg1.size() / 2, 0, msg1, color_t
	{ 0, 255, 255 }, color_t
	{ 0, 0, 0 });
	...
	for (const solar_system_body_t &body : finder->second.bodies)
	{
		...
		if (body.body_type == ICE_WORLD)
		{
			...
			vterm::set_char_xy(10 + (i * 4), (size.second / 2) - 1,
					render_char);
			vterm::set_char_xy(10 + (i * 4), (size.second / 2) + 1,
					render_char);
		}
		else if (body.body_type == VOLCANIC_WORLD)
		{
			vterm::screen_character render_char
			{ 219, color_t
			{ 255, game_engine->rng.roll_dice(1, 255), 0 }, color_t
			{ 0, 0, 0 } };
			vterm::set_char_xy(9 + (i * 4), (size.second / 2), render_char);
			vterm::set_char_xy(10 + (i * 4), (size.second / 2), render_char);
			vterm::set_char_xy(11 + (i * 4), (size.second / 2), render_char);
		...
		else if (body.body_type == HABITABLE_PLANET)
		{
			...
			{ 0, 255, 0 }, color_t
			{ 0, 0, 0 } };
			vterm::screen_character render_char2
			{ 219, color_t
			{ 0, 0, 255 }, color_t
			{ 0, 0, 0 } };
			vterm::set_char_xy(9 + (i * 4), (size.second / 2), render_char);
...
void genesis_mode::show_universe_map(const bool show_warzone,
		const bool show_truce)
{
	...
	for (int y = 0; y < UNIVERSE_HEIGHT; ++y)
	{
		for (int x = 0; x < UNIVERSE_WIDTH; ++x)
		{
			...
			color_t background
			{ 0, 0, 0 };
			if (show_warzone and distance < size_of_war)
				background = color_t
				{ 64 + game_engine->rng.roll_dice(1, 64), 0, 0 };
			if (show_truce and distance < 5)
				background = color_t
				{ 0, 128, 0 };
			...
			if (finder != universe->solar_systems.end())
			{
				...
				std::pair<SDL_Color, unsigned char> renderable =
						solar_system_renderable(finder->second);
				vterm::screen_character render_char
				{ renderable.second, color_t
				{ renderable.first.r, renderable.first.g, renderable.first.b },
						background };
				if (show_warzone and distance < size_of_war)
