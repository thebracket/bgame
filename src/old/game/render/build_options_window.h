...
private:
			...
			do_nothing, const int &fill_lines = 0)
	{
		...
		line_t line
		{ s, col, raws::get_tile_source(icon_idx), size.first, hotspot, func,
				fill_lines };
...
public:
	build_options_window(sdl2_backend* sdl, const string& title,
			const bool& decor, const int rx, const int ry, const int rz) :
			window(sdl, title, decor), region_x(rx), region_y(ry), region_z(rz)
	{
		...
		for (const line_t &panel_line : lines)
		{
			...
			SDL->render_bitmap("spritesheet", panel_line.icon_source, SDL_Rect
			{ 0, y, 16, 16 });
	...
	void determine_buildables(const int &idx)
	{
		...
		if (world::current_region->tiles[idx].base_tile_type != tile_type::FLAT
				and world::current_region->tiles[idx].base_tile_type
						!= tile_type::BEACH)
		{
		...
		{
			add_line(
					"There is already a structure here: demolish it before building a new one.",
					sdl_white);
			return;
		}
		...
		for (const tuple<string, int, vector<string>> &buildable : buildables)
		{
			...
			if (have_components)
			{
				...
				{
					std::stringstream ss;
					ss << "  Requires: " << component << " (" << available
							<< " available)";
