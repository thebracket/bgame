...
void render_settler_composite(sdl2_backend* SDL, const int& entity_id,
		const int& x, const int& y)
{
	game_species_component * species = ECS->find_entity_component<
			game_species_component>(entity_id);
	...
	case skin_color_t::AFRICAN:
		SDL->set_color_mod("spritesheet", 128, 58, 45);
		break;
	case skin_color_t::ASIAN:
		SDL->set_color_mod("spritesheet", 199, 151, 120);
		break;
	case skin_color_t::INDIAN:
		SDL->set_color_mod("spritesheet", 179, 121, 100);
		break;
	}
	SDL->render_bitmap("spritesheet", src, dest);
	...
	for (const item_carried_component item_c : *carried)
	{
		...
		{
			const item_component * item = ECS->find_entity_component<
					item_component>(item_c.entity_id);

			if (item != nullptr and item->clothing_slot > 0)
			{
				const renderable_component * item_r =
						ECS->find_entity_component<renderable_component>(
								item_c.entity_id);
				src = raws::get_tile_source(item_r->tile_idx);
				if (item->is_tinted)
				{
					SDL->set_color_mod("spritesheet", item->tint_red,
							item->tint_green, item->tint_blue);
				}
				else
				{
					SDL->set_color_mod("spritesheet", 255, 255, 255);
