...
struct texture_resource
{
	...
	texture_resource(SDL_Renderer * renderer, const std::string &fn,
			const std::string &tag) :
			filename(fn), name(tag)
	{
...
struct font_resource
{
	...
	font_resource(const std::string &fn, const std::string &tag,
			const int &size) :
			filename(fn), name(tag), point_size(size)
	{
...
struct sdl_resource_manager
{
	...
	void load_image(SDL_Renderer * renderer, const std::string &filename,
			const std::string &tag)
	{
	...
	void load_font(const std::string &filename, const std::string &tag,
			const int &size)
	{
	...
	std::string render_text_to_texture(SDL_Renderer * renderer,
			const std::string &font_tag, const std::string text,
			const std::string &new_tag, SDL_Color color)
	{
		...
		if (surf == nullptr)
		{
			std::cout << "Null surface when rendering [" << text << "]:"
					<< TTF_GetError() << "\n";
			throw 101;
		}
	...
	std::string create_temporary_texture(SDL_Renderer * renderer,
	...
	std::pair<int, int> text_size(const std::string &font,
