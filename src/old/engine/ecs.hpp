...
public:
	...
	inline void profile_timer(const int &t)
	{
		...
		if (t < min_execution_time)
			min_execution_time = t;
	}
};
...
public:
	 ...
	 * serialization. */
	entity_component_system(const function<void(fstream &, const int &)> loader,
			function<void(fstream &)> world_loader,
			function<void(fstream &)> world_saver) :
			loader_callback(loader), load_constants(world_loader), save_constants(
					world_saver)
	{
	}
	...
	vector<tuple<string, int, int, int>> get_profile_info()
	{
		...
		{
			result.push_back(
					std::make_tuple(system->system_name,
							system->last_execution_time,
							system->min_execution_time,
	...
	void load_game(const string filename)
	{
		...
		lbfile.read(reinterpret_cast<char *>(&number_of_entities),
				sizeof(number_of_entities));
		for (int i = 0; i < number_of_entities; ++i)
		{
			entity e = construct_entity_from_file(lbfile);
			entities.add_entity(e);
			//std::cout << "Loaded entity #" << e.handle << "\n";
			if (entities.next_handle <= e.handle)
				entities.next_handle = e.handle + 1;
		}

		int number_of_components = 0;
		lbfile.read(reinterpret_cast<char *>(&number_of_components),
				sizeof(number_of_components));
	...
	{
		fstream lbfile(filename,
				std::ios::out | std::ios::binary | std::ios::trunc);

		save_constants(lbfile);

		int number_of_entities = entities.size();
		lbfile.write(reinterpret_cast<const char *>(&number_of_entities),
				sizeof(number_of_entities));
		...
		lbfile.write(reinterpret_cast<const char *>(&number_of_components),
				sizeof(number_of_components));
