...
void sdl2_backend::render_bitmap_centered(const string& tag, const int& x, const int& y)
{
	...
	SDL_QueryTexture(bmp, NULL, NULL, &image_width, &image_height);
	SDL_Rect target
	{ x - (image_width / 2), y, image_width, image_height };
	SDL_Rect src
	{ 0, 0, image_width, image_height };
	//std::cout << image_width << " x " << image_height << "\n";
	SDL_RenderCopy(renderer, bmp, &src, &target);
...
void sdl2_backend::set_color_mod(const string& tag, unsigned char r,
...
string sdl2_backend::create_temporary_texture(const int& width,
		const int& height)
{
...
void sdl2_backend::draw_vterm(vector<vterm::screen_character>* screen)
{
	...
	{ 176, 208, TERMINAL_SIZE, TERMINAL_SIZE };
...
void sdl2_backend::poll_inputs()
{
	...
	while (SDL_PollEvent(&e) != 0)
	{
		...
		{
			command::on_command(
			{ translate_keycode(e), 0, 0, command::KEYDOWN });
		}
		if (e.type == SDL_MOUSEMOTION)
		{
			command::on_command(
					{ command::keys::NONE, e.motion.x, e.motion.y,
							command::MOUSE_MOVE });
		}
		if (e.type == SDL_MOUSEBUTTONDOWN)
		{
			if (e.button.button == SDL_BUTTON_LEFT)
				command::on_command(
				{ command::keys::NONE, 0, 0, command::MOUSE_LEFT_CLICK });
			if (e.button.button == SDL_BUTTON_RIGHT)
				command::on_command(
				{ command::keys::NONE, 0, 0, command::MOUSE_RIGHT_CLICK });
		}
	}
}
