...
void set_char_xy(const int x, const int y,
		const engine::vterm::screen_character& c)
{
...
void print(const int x, const int y, const string text, const color_t fg,
		color_t bg)
{
	...
	{
		set_char(idx + i,
		{ static_cast<const unsigned char>(text[i]), fg, bg });
	}
}
...
void draw_dbl_box(const int &x, const int &y, const int &w, const int &h,
		const color_t fg, const color_t bg)
{
	set_char_xy(x, y,
	{ 201, fg, bg });
...
{
	// Temporary
	engine::vterm::terminal_buffer = make_unique<
			vector<engine::vterm::screen_character>>();
