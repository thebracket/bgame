...
{
public:
	scene_background(const std::string tag) :
			image_tag(tag)
	{
	}
	...
	{
		const SDL_Rect big_rect
		{ 0, 0, 1024, 768 };
		const auto screen_size = SDL->get_screen_size();
		const SDL_Rect full_screen
		{ 0, 0, screen_size.first, screen_size.second };
...
{
public:
	scene_blit(const std::string &tag, const SDL_Rect &source,
			const SDL_Rect &dest) :
			image_tag(tag), src(source), dst(dest)
	{
	}
	std::string image_tag;
	SDL_Rect src;
...
{
public:
	scene_blit_centered(const std::string &tag, const int &X, const int &Y) :
			image_tag(tag), x(X), y(Y)
	{
	}
	std::string image_tag;
	int x, y;
