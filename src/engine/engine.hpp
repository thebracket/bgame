...
{
public:
	bracket_engine(const function<void(fstream &, const int &)> loader,
			function<void(fstream &)> world_loader,
			function<void(fstream &)> world_saver) noexcept
	{
		ecs = new entity_component_system<typename component_list::type_list>(
				loader, world_loader, world_saver);
...
public:
	void init(const std::string &window_title, const int width = 1024,
	...
	void main_loop(unique_ptr<base_mode> starting_mode)
	{
		...
		while (!quitting)
		{
			...
			if (continuation_mode.first == POP)
				pop_mode();
	...
	std::string render_text_to_image(const std::string &font_tag,
			const std::string text, const std::string &new_tag, SDL_Color color)
	{
		return backend_driver.render_text_to_image(font_tag, text, new_tag,
				color);
	}
};
