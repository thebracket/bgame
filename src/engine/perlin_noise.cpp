...
perlin_noise::perlin_noise(double _persistence, double _frequency,
		double _amplitude, int _octaves, int _randomseed)
{
	...
	octaves_i = _octaves;
	random_seed_i = 2 + _randomseed * _randomseed;
}

void perlin_noise::set(double _persistence, double _frequency,
		double _amplitude, int _octaves, int _randomseed)
{
	persistence_d = _persistence;
	frequency_d = _frequency;
...
double perlin_noise::total(double i, double j) const
{
	...
	double t = 0.0f;
	double _amplitude = 1;
	double freq = frequency_d;

	for (int k = 0; k < octaves_i; k++)
	{
		t += get_value(j * freq + random_seed_i, i * freq + random_seed_i)
				* _amplitude;
		_amplitude *= persistence_d;
		freq *= 2;
	}
