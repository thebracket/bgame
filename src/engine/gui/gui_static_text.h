...
public:
	...
	gui_static_text(const string &name, const string &txt, const int &nx,
			const int &ny, const vterm::color_t &fg, const vterm::color_t &bg,
			bool center = false) :
			centered(center), text(txt), x(nx), y(ny), foreground(fg), background(
					bg)
