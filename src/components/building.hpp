...
struct building_t {
    building_t(const std::string ntag, const int w, const int h, const std::vector<rltk::vchar> &g,
			   const std::vector<rltk::vchar> &ga, const int &model, const bool comp, const std::size_t owner,
			   uint8_t maxhp, uint8_t hp) :
        tag(ntag), width(w), height(h), glyphs(g), glyphs_ascii(ga), model_idx(model), complete(comp), civ_owner(owner),
	...
	{
		archive( tag, width, height, glyphs, glyphs_ascii, model_idx, complete,
				 built_with, civ_owner, max_hit_points, hit_points ); // serialize things by passing them to the archive
	}
