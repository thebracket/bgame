...
void spawn_item_on_ground(const int x, const int y, const int z, const std::string &tag, const std::size_t &material, 
	uint8_t quality=3, uint8_t wear=100, std::size_t creator_id=0, std::string creator_name="") noexcept;

bengine::entity_t * spawn_item_on_ground_ret(const int x, const int y, const int z, const std::string &tag, 
	const std::size_t &material, uint8_t quality = 3, uint8_t wear = 100, std::size_t creator_id = 0, std::string creator_name = "") noexcept;

void spawn_item_in_container(const std::size_t container_id, const std::string &tag, const std::size_t &material, 
	uint8_t quality = 3, uint8_t wear = 100, std::size_t creator_id = 0, std::string creator_name = "") noexcept;

void spawn_item_carried(const std::size_t holder_id, const std::string &tag, const std::size_t &material, const 
	item_location_t &loc, uint8_t quality = 3, uint8_t wear = 100, std::size_t creator_id = 0, std::string creator_name = "") noexcept;
