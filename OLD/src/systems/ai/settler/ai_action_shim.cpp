...
void ai_action_shim::update(const double duration_ms) {
    ...
    each<ai_tag_my_turn_t, ai_mode_idle_t, settler_ai_t>([] (entity_t &e, ai_tag_my_turn_t &t, ai_mode_idle_t &i, settler_ai_t &s) {
        emit(action_available_message{e.id});
        delete_component<ai_tag_my_turn_t>(e.id);
    });

    each<ai_tag_my_turn_t, ai_tag_work_shift_t, settler_ai_t>([] (entity_t &e, ai_tag_my_turn_t &t, ai_tag_work_shift_t &i, settler_ai_t &s) {
        emit(action_available_message{e.id});
        delete_component<ai_tag_my_turn_t>(e.id);
